...
# TASK:
# Develop a machine learning model that predicts house prices with minimum error using the available dataset
...
################################################################
# Load and merge the training and test datasets. Proceed with the merged data.
################################################################

train = pd.read_csv("machine_learning/datasets/train.csv")
test = pd.read_csv("machine_learning/datasets/test.csv")
...
# Investigating the Logarithm of the Dependent Variable
...
# The missing values in some variables indicate that the house does not have that feature.
no_cols = ["Alley", "BsmtQual", "BsmtCond", "BsmtExposure", "BsmtFinType1", "BsmtFinType2", "FireplaceQu",
           "GarageType", "GarageFinish", "GarageQual", "GarageCond", "PoolQC", "Fence", "MiscFeature"]

# Filling the empty spaces in columns with the expression 'No'
for col in no_cols:
...
def quick_missing_imp(data, num_method="median", cat_length=20, target="SalePrice"):
    variables_with_na = [col for col in data.columns if
                         data[col].isnull().sum() > 0]  # Listing variables with missing values

    temp_target = data[target]

    print("# BEFORE")
    print(data[variables_with_na].isnull().sum(), "\n\n")
    # The number of missing values in variables before applying the procedure

    # If the number of unique classes in the 'object' variable is equal to or less than the cat_length,
    # fill the missing values with mode
    data = data.apply(lambda x: x.fillna(x.mode()[0]) if (x.dtype == "O" and len(x.unique()) <= cat_length) else x,
                      axis=0)

    # If the num_method is mean, missing values of non-object type variables are filled with the mean
    if num_method == "mean":
        data = data.apply(lambda x: x.fillna(x.mean()) if x.dtype != "O" else x, axis=0)
    # If the num_method is median, missing values of non-object type variables are filled with the median
    elif num_method == "median":
        data = data.apply(lambda x: x.fillna(x.median()) if x.dtype != "O" else x, axis=0)

    data[target] = temp_target # empty sale prices should not be filled
...
######################################
# Create new variables and add 'NEW' to the beginning of the names of the newly created variables
######################################
...
# Dropping variables in the drop_list
df.drop(drop_list, axis=1, inplace=True)

##################
# Apply Label Encoding & One-Hot Encoding to the dataset
##################
...
#  (The observations with missing values in the SalePrice variable will be considered as test data.)
...
# Build a model using the training data and evaluate its performance
...
##################
# Build a model using logarithmic transformation and observe the RMSE results.
# Note: Do not forget to take the inverse of the logarithm.
...
y_pred
# Taking the inverse of the performed logarithmic transformation
...
################################################################
# "Visualize the ranking of features using the feature_importance function,
# which determines the importance level of variables.
################################################################
...
plot_importance(model, X, num=7)

########################################
# Make predictions for the missing SalePrice variables in the test dataframe and create a dataframe suitable for
# submission to the Kaggle page (Id, SalePrice)
########################################
